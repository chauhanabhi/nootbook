  ...
  useEffect(() => {
    async function fetchProducts() {
      try{
          ...
          const filterProduct = searchTerm ? data.filter(item =>
            item.name.toLowerCase().includes(searchTerm.toLowerCase())
          )
            : data
   
         initialProductList(filterProduct); 
      }
      catch(error){
    ...
    <main className='container mx-auto'>
      <section className="my-5">
        <div className="my-5 flex justify-between">
          <span className="text-2xl font-semibold dark:text-slate-100 mb-5">All {searchTerm} eBooks - {productList.length}</span>
          <span>
